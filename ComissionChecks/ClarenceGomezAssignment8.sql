...
INSERT INTO `employee` VALUES (1,'Prince','Igor','12 Creekside St','92647','2025550109','iprince@isp.com'),(2,'Rennie','Dante','137 Wall St','38117','2025550112','drennie@isp.com'),(3,'Waters','Thomas','411 Shirley St','24210','2025550115','twaters@isp.com'),(4,'Clarke','Avaya','7771 Border Court','92078','2025550119','aclarke@isp.com'),(5,'Kaiser','Haniya','6 South Sulphur Springs Street','70121','2025550121','hkaiser@isp.com'),(6,'Newman','Victor','942 New Saddle Drive','70433','2025550122','vnewman@isp.com'),(7,'Guthrie','Douglas','9 Wrangler Ave','91764','2025550124','dguthrie@isp.com'),(8,'Roberts','Brian','7319 S. Greenview Drive','44223','2025550125','broberts@isp.com'),(9,'Solis','Paris','763 West Mulberry St','49016','2025550132','psolis@isp.com'),(10,'Heath','Umaiza','545 Ohio Ave','85260','2025550135','uheath@isp.com');
...
/*!50001 VIEW `customerview` AS select `customer`.`firstName` AS `firstName`,`customer`.`lastName` AS `lastName`,`customer`.`address` AS `address`,`citystate`.`city` AS `city`,`citystate`.`state` AS `state`,`customer`.`zipCode` AS `zipCode`,`customer`.`phone` AS `phone`,`customer`.`email` AS `email` from (`customer` join `citystate`) where (`customer`.`zipCode` = `citystate`.`zipCode`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `employeeview` AS select `employee`.`firstName` AS `firstName`,`employee`.`lastName` AS `lastName`,`employee`.`address` AS `address`,`citystate`.`city` AS `city`,`citystate`.`state` AS `state`,`employee`.`zipCode` AS `zipCode`,`employee`.`phone` AS `phone`,`employee`.`email` AS `email` from (`employee` join `citystate`) where (`employee`.`zipCode` = `citystate`.`zipCode`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `saleview` AS select concat(`c`.`firstName`,' ',`c`.`lastName`) AS `Customer Name`,concat(`c`.`address`,' ',`cs`.`city`,' ',`cs`.`state`,' ',`c`.`zipCode`) AS `Customer Address`,`c`.`phone` AS `Customer Phone`,`c`.`email` AS `Customer Email`,concat(`e`.`firstName`,' ',`e`.`lastName`) AS `Sales Associate`,`e`.`phone` AS `Sales Associate Phone`,`e`.`email` AS `Sales Associate Email`,`v`.`year` AS `Year`,`m`.`make` AS `Make`,`mo`.`model` AS `Model`,`co`.`color` AS `Color`,`t`.`type` AS `Type`,`v`.`vin` AS `VIN`,`s`.`salePrice` AS `Sale Price` from ((((((((`customer` `c` join `employee` `e`) join `sale` `s`) join `citystate` `cs`) join `vehicle` `v`) join `make` `m`) join `model` `mo`) join `color` `co`) join `type` `t`) where ((`s`.`customerId` = `c`.`ID`) and (`s`.`employeeId` = `e`.`ID`) and (`s`.`vehicleId` = `v`.`ID`) and (`c`.`zipCode` = `cs`.`zipCode`) and (`v`.`make` = `m`.`ID`) and (`v`.`model` = `mo`.`ID`) and (`v`.`color` = `co`.`ID`) and (`v`.`type` = `t`.`ID`)) order by `v`.`ID` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2022-12-04 18:29:07
